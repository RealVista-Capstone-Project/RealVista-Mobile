import Svg, { ClipPath, Defs, G, Mask, Path, Rect } from 'react-native-svg'

type NotificationIconProps = {
  width?: number
  height?: number
  color?: string
}

export function NotificationIcon({
  width = 24,
  height = 24,
  color = '#000929',
}: NotificationIconProps) {
  return (
    <Svg width={width} height={height} viewBox='0 0 24 24' fill='none'>
      <G clipPath='url(#clip0_6377_14637)'>
        <Path
          d='M5.26855 9.74994C5.26731 8.86045 5.44213 7.97951 5.78294 7.1579C6.12375 6.33628 6.62381 5.59025 7.25429 4.9628C7.88476 4.33535 8.6332 3.83889 9.45644 3.50204C10.2797 3.16519 11.1615 2.99461 12.0509 3.00013C15.7625 3.02772 18.7312 6.11275 18.7312 9.83468V10.4999C18.7312 13.8577 19.4337 15.8061 20.0524 16.871C20.1191 16.9848 20.1546 17.1142 20.1553 17.246C20.156 17.3779 20.1219 17.5076 20.0565 17.6221C19.991 17.7366 19.8966 17.8318 19.7826 17.8982C19.6686 17.9645 19.5392 17.9996 19.4073 17.9999H4.59173C4.45985 17.9996 4.33038 17.9645 4.2164 17.8981C4.10242 17.8318 4.00795 17.7366 3.94252 17.622C3.8771 17.5075 3.84303 17.3778 3.84376 17.2459C3.84449 17.114 3.87999 16.9846 3.94667 16.8709C4.56573 15.8059 5.26855 13.8575 5.26855 10.4999L5.26855 9.74994Z'
          stroke={color}
          strokeWidth={2}
          strokeLinecap='round'
          strokeLinejoin='round'
        />
        <Path
          d='M9 18V18.75C9 19.5456 9.31607 20.3087 9.87868 20.8713C10.4413 21.4339 11.2044 21.75 12 21.75C12.7956 21.75 13.5587 21.4339 14.1213 20.8713C14.6839 20.3087 15 19.5456 15 18.75V18'
          stroke={color}
          strokeWidth={2}
          strokeLinecap='round'
          strokeLinejoin='round'
        />
        <Mask
          id='path-3-outside-1_6377_14637'
          maskUnits='userSpaceOnUse'
          x={12}
          y={2}
          width={10}
          height={10}
        >
          <Rect fill='white' x={12} y={2} width={10} height={10} />
          <Path
            fillRule='evenodd'
            clipRule='evenodd'
            d='M17 4C18.6569 4 20 5.34315 20 7C20 8.65685 18.6569 10 17 10C15.3431 10 14 8.65685 14 7C14 5.34315 15.3431 4 17 4Z'
          />
        </Mask>
        <Path
          fillRule='evenodd'
          clipRule='evenodd'
          d='M17 4C18.6569 4 20 5.34315 20 7C20 8.65685 18.6569 10 17 10C15.3431 10 14 8.65685 14 7C14 5.34315 15.3431 4 17 4Z'
          fill='#7065F0'
        />
        <Path
          d='M20 7H22C22 4.23858 19.7614 2 17 2V4V6C17.5523 6 18 6.44772 18 7H20ZM17 10V12C19.7614 12 22 9.76142 22 7H20H18C18 7.55228 17.5523 8 17 8V10ZM14 7H12C12 9.76142 14.2386 12 17 12V10V8C16.4477 8 16 7.55228 16 7H14ZM17 4V2C14.2386 2 12 4.23858 12 7H14H16C16 6.44772 16.4477 6 17 6V4Z'
          fill='#F7F7FD'
          mask='url(#path-3-outside-1_6377_14637)'
        />
      </G>
      <Defs>
        <ClipPath id='clip0_6377_14637'>
          <Rect width={24} height={24} fill='white' />
        </ClipPath>
      </Defs>
    </Svg>
  )
}
