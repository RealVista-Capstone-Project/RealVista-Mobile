...
// Helper function to format price in Vietnamese format
const formatPrice = (price: number): string => {
  if (price >= 1000000000) {
    ...
    return millionValue % 1 === 0 ? `${millionValue} tr` : `${millionValue.toFixed(1)} tr`
  } else {
    return price >= 1000 ? `${(price / 1000).toFixed(0)}K` : price.toFixed(0)
  }
}
...
export const LineChart = ({ data, config = {}, style }: Props) => {
  ...
  const mutedColor = BRAND_COLORS.textMuted
  ...
  // Fixed animated props for SVG components - must be before early return
